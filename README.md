# SINGULARITY 2

Five players. **One body.** A chaotic co-op physics party game: someone steers the head, someone works the arms, someone keeps the torso balanced, and two people each own a leg. Walk, climb, grab, throw — and try not to fall in the water.

This is a rebuild of the original *Many Hands* prototype with a real realtime backend: all multiplayer state (rooms, teams, roles, inputs, physics snapshots, leaderboards) lives in **[SpacetimeDB](https://spacetimedb.com)** instead of in-memory Next.js API routes + SSE polling.

## Architecture

```
┌─────────────────────────────┐         wss          ┌──────────────────────────────┐
│  Next.js 16 + React 19      │  ←────────────────→  │  SpacetimeDB module (TS)     │
│  Three.js + Rapier physics  │   subscriptions +    │  server/src/index.ts         │
│  (client-side simulation)   │   reducer calls      │  rooms · teams · timers      │
└─────────────────────────────┘                      └──────────────────────────────┘
```

- **`server/`** — SpacetimeDB server module written in TypeScript (`spacetime build`). Owns:
  - **Rooms & teams** — join by 4-letter code, auto team/role assignment, host election, ready-up.
  - **Round lifecycle** — `start` → scheduled reducer flips `countdown → playing` after 4.2 s; 45 s grace timer after the first team finishes; results → lobby.
  - **Input relay** — non-host players write per-role inputs; the team's *host client* (which simulates the ragdoll with Rapier) consumes them.
  - **Snapshot relay** — the host publishes body/prop transforms at ~15 Hz; teammates interpolate, other teams render as ghosts.
  - **Leaderboard** — finished times are inserted server-side into a `score` table.
  - **Feedback** — private, idempotent feedback records are written through a validated reducer.
  - **Cleanup** — connections are tracked; players/rooms are removed when the last connection drops (plus a stale-player sweeper).
- **`src/`** — the Next.js app. `src/game/net.ts` wraps the generated SpacetimeDB bindings (`src/module_bindings/`, generated by `spacetime generate`) behind the same interface the game engine has always used. Game code (`game.ts`, `body.ts`, physics, rendering) is unchanged from the original prototype.
- **`src/app/api/feedback`** — validates homepage feedback, stores it in SpacetimeDB, and sends a thank-you through Resend without exposing the API key to the browser.

## Repo layout

```
spacetime.json          points the CLI at ./server (maincloud by default)
server/                 SpacetimeDB module (TypeScript)
src/                    Next.js app (game engine + UI)
src/module_bindings/    generated client bindings — do not edit
scripts/e2e.ts          end-to-end test for the module (2 simulated players)
```

## Develop

```bash
npm install                      # app deps
cd server && npm install && cd ..# module deps

spacetime start --listen-addr 127.0.0.1:3007   # local SpacetimeDB (optional)
npm run spacetime:publish:local                # or: spacetime publish --module-path server --server <url>
npm run spacetime:generate                     # regenerate src/module_bindings after module changes
npm run dev                                    # http://localhost:3000
```

Copy `.env.example` to `.env.local` and set `RESEND_API_KEY` plus
`RESEND_FROM_EMAIL`. The sender must use a domain verified in Resend; the default
`onboarding@resend.dev` sender can only email the address that owns the Resend
account and is therefore only useful for a one-recipient test.

Run the module test suite:

```bash
npx esbuild scripts/e2e.ts --bundle --platform=node --format=esm --outfile=scripts/e2e.mjs
STDB_URI=ws://127.0.0.1:3007 node scripts/e2e.mjs
```

## Deploy

**Backend → SpacetimeDB Maincloud** (free hosted cloud):

```bash
spacetime login
npm run spacetime:publish        # publishes ./server as database "singularity2-sankalphs"
```

**Frontend → Vercel**: import the repo, framework *Next.js*. The client connects to
`wss://maincloud.spacetimedb.com` and database `singularity2-sankalphs` by default; override with
`NEXT_PUBLIC_SPACETIMEDB_URI` / `NEXT_PUBLIC_SPACETIMEDB_MODULE` if you self-host or rename.
Add `SPACETIMEDB_HTTP_URI`, `SPACETIMEDB_MODULE`, `RESEND_API_KEY`, and
`RESEND_FROM_EMAIL` to the Vercel project environment. Publish the SpacetimeDB
module before deploying the frontend so the `submit_feedback` reducer exists.

The module applies a durable per-recipient feedback limit, while the Next.js
route also applies a best-effort per-instance IP limit. For a high-traffic public
launch, put a distributed edge rate limiter or server-verified CAPTCHA in front
of the route as an additional abuse control.

