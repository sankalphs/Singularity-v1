...
This is a rebuild of the original *Many Hands* prototype with a real realtime backend: all multiplayer state (rooms, teams, roles, inputs, physics snapshots, leaderboards) lives in **[SpacetimeDB](https://spacetimedb.com)** instead of in-memory Next.js API routes + SSE polling.
...
- **`src/`** — the Next.js app. `src/game/net.ts` wraps the generated SpacetimeDB bindings (`src/module_bindings/`, generated by `spacetime generate`) behind the same interface the game engine has always used. Game code (`game.ts`, `body.ts`, physics, rendering) is unchanged from the original prototype.
- **`src/app/api/feedback`** — validates homepage feedback, stores it in SpacetimeDB, sends a thank-you to the player, and forwards the note to a private inbox through Resend without exposing the API key to the browser.

## Repo layout

```
spacetime.json          points the CLI at ./server (maincloud by default)
...
`RESEND_FROM_EMAIL`, and `FEEDBACK_INBOX_EMAIL`. The sender must use a domain
...
**Backend → SpacetimeDB Maincloud** (free hosted cloud):
...
`wss://maincloud.spacetimedb.com` and database `singularity2-sankalphs` by default; override with
`NEXT_PUBLIC_SPACETIMEDB_URI` / `NEXT_PUBLIC_SPACETIMEDB_MODULE` if you self-host or rename.
Add `SPACETIMEDB_HTTP_URI`, `SPACETIMEDB_MODULE`, `RESEND_API_KEY`,
`RESEND_FROM_EMAIL`, and `FEEDBACK_INBOX_EMAIL` to the Vercel project
environment. Publish the SpacetimeDB module before deploying the frontend so
the `submit_feedback` reducer exists.
